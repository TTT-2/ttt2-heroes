...
function HEROES.HeroActivate()
	...
	if ply.heroOpt1 and GetGlobalBool("ttt_heroes_option") then
	...
	if not ply:HasCrystal() or not ply:IsHero() then return end
...
function HEROES.AbortHero()
	...
	if ply.heroOpt2 and GetGlobalBool("ttt_heroes_option") then
		net.Start("TTTHChooseHeroOption")
		net.WriteBool(true)
		net.SendToServer()

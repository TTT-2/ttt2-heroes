...
if CLIENT then
	...
	function HUDELEMENT:GetDefaults()
		const_defaults["basepos"] = { x = ScrW() - self.optionSize - self.optionMargin, y = ScrH() - self.optionSize - self.optionMargin }
	...
	function HUDELEMENT:Draw()
		...
		local crystalValid = client:HasCrystal() -- TODO to improve performance, use a var and just update it on destruction
		local text, icon

		if not crystalValid and client:GetNWBool("CanSpawnCrystal") then
			oldTimerVal = nil
			icon = diamondMaterial
		elseif hd.deactivated or not crystalValid and not client:GetNWBool("CanSpawnCrystal") or hd.amount and hd.amount <= (client.heroAmount or 0) then
			oldTimerVal = nil
			text = "-"
		...
		elseif text then
			...
			if w >= self.optionSize then
				draw.SimpleText(text, "Trebuchet24", x + hw, y + hw, Color(255, 255, 255, 150), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			else
				draw.SimpleText(text, "Trebuchet18", x + hw, y + hw, Color(255, 255, 255, 150), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			end
		end
	end
end

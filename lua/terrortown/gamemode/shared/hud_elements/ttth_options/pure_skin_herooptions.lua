...
if CLIENT then
	...
	function HUDELEMENT:DrawHeroOption(ty, key, name, color)
		...
		draw.SimpleText(key, "HeroDescOptions", x + pad * 0.5, ty + self.optionHeight * 0.5, self:GetDefaultFontColor(color), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

		-- draw line
		local line = 3

		draw.RoundedBoxEx(0, x + pad + 1, ty + 5, 1, self.optionHeight - 10, self:GetDefaultFontColor(color))

		-- draw hero name
		draw.SimpleText(name, "HeroDesc", x + pad + line + (w - pad - line) * 0.5, ty + self.optionHeight * 0.5, self:GetDefaultFontColor(color), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	...
	function HUDELEMENT:Draw()
		...
		local hd1 = HEROES.GetHeroDataByIndex(client.heroOpt1)
		local hd2 = HEROES.GetHeroDataByIndex(client.heroOpt2)
	...
	function HUDELEMENT:ShouldDraw()
		...
		return client:IsActive() and GetGlobalBool("ttt_heroes_option") and client.heroOpt1 and client.heroOpt2
	end
end
